...
def gatherAnnotations(labelDirectory):
    dataFrameLabel = pd.DataFrame()
    dirContent = os.scandir(labelDirectory)

    for element in dirContent:
        if element.name.split(".")[1] == "xml":
            annotationData = annotationFromXml(element.path)
